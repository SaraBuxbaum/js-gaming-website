//הכנסת כל הצלילים לתוך משתנים בשמות הצלילים ע''י האובייקט אודיו שמקבל רצועת שמע
...
//פונקציה שאחראית על הפעלת הצליל המבוקש
const playSound = audio => 
{
  
  const clone = audio.cloneNode();
  clone.play();
  setTimeout(() => (clone.volume = 0.8), 400);//מהו אורך השמע הרצוי ומה יהיה הווליום
...
function piano()
{
  window.location="../HTML/piano.html"
}

//שורה ראשונה- קבלת הקליד שהקשנו והכנסתו למשתנה
//יצירת פונקציה קבועה
//תשמיע את הצליל המבוקש
//הצליל יהיה פעיל במשך 200 מילישניות ואח''כ הוא יוסר מהרשימה ולא יהיה פעיל
//כנ''ל לגבי כל הפונקציות הבאות
...
const playE5 = () => {
  playSound(E5);
  E5Key.classList.add("active",54);
  setTimeout(() => E5Key.classList.remove("active"), 200);
};
E5Key.addEventListener("click", playE5);

//בעת לחיצה מטה על המקש-נקבל את הצליל התואם(מיקום)
//הפונקציה הבאה היא פונקציה שמורה בשפה שמקבלת את סוג האירוע שעליו אנו מאזינים
//וכן מקבלת את הפונקציה שתופעל ברגע שהאירוע יתרחש
